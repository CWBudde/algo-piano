...
func TestInitCandidateBodyIRMix(t *testing.T) {
	...
	// body-ir: 6 knobs, dual-IR mix (because body-ir triggers needsIRSynthesis): 4 knobs = 10 total
	if len(defs) != 10 {
		t.Fatalf("defs len = %d, want 10", len(defs))
	}
	if len(cand.Vals) != len(defs) {
		t.Fatalf("vals len = %d, want %d", len(cand.Vals), len(defs))
	}
...
func TestInitCandidateFullJoint(t *testing.T) {
	...
	// piano: 13, body-ir: 6, room-ir: 7, dual-IR mix: 4 = 30 total
	if len(defs) != 30 {
		t.Fatalf("defs len = %d, want 30", len(defs))
	}
	if len(cand.Vals) != len(defs) {
		t.Fatalf("vals len = %d, want %d", len(cand.Vals), len(defs))
	}
