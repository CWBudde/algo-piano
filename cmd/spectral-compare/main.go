...
func main() {
	...
	// The norm divisors come from the analysis package rather than being
	// repeated here: a literal would keep printing the old scale the moment a
	// norm moves, and quietly report a percentage that is no longer true.
	fmt.Printf("  TimeRMSE:        %.4f  (norm'd to %g → %.1f%%)\n",
		m.TimeRMSE, analysis.NormTime, clamp01(m.TimeRMSE/analysis.NormTime)*100)
	fmt.Printf("  EnvelopeRMSEDB:  %.1f dB (norm'd to %g → %.1f%%)\n",
		m.EnvelopeRMSEDB, analysis.NormEnvelope, clamp01(m.EnvelopeRMSEDB/analysis.NormEnvelope)*100)
	fmt.Printf("  SpectralRMSEDB:  %.1f dB (norm'd to %g → %.1f%%)\n",
		m.SpectralRMSEDB, analysis.NormSpectral, clamp01(m.SpectralRMSEDB/analysis.NormSpectral)*100)
	fmt.Printf("    low(0-500Hz):  %.1f dB  mid(500-2k): %.1f dB  high(2k+): %.1f dB\n",
		m.SpectralLowRMSEDB, m.SpectralMidRMSEDB, m.SpectralHighRMSEDB)
	fmt.Printf("  DecayDiffDBPerS: %.1f dB/s (norm'd to %g → %.1f%%)\n",
		m.DecayDiffDBPerS, analysis.NormDecay, clamp01(m.DecayDiffDBPerS/analysis.NormDecay)*100)
	if m.SpectralSaturated {
		fmt.Printf("  WARNING: spectral component saturated (%.1f dB >= NormSpectral %g) - this component provides no gradient\n",
			m.SpectralRMSEDB, analysis.NormSpectral)
	}
